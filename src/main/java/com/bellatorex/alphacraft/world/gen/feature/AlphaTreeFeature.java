...
import java.util.*;
public class AlphaTreeFeature extends Feature<BaseTreeFeatureConfig> {

    public AlphaTreeFeature(Codec<BaseTreeFeatureConfig> p_i231999_1_) {
        super(p_i231999_1_);
    }
    ...
    private static boolean isValidToPlace(IWorldGenerationBaseReader p_236418_0_, BlockPos p_236418_1_) {
        return p_236418_0_.hasBlockState(p_236418_1_, (p_236409_0_) -> {
            Block block = p_236409_0_.getBlock();
            return block == Blocks.END_STONE ||
                    block == BlockRegistry.END_GRASS_BLOCK.get() ||
                    block == BlockRegistry.DARK_DIRT_BLOCK.get() ||
                    block == BlockRegistry.DARK_GRASS_BLOCK.get() ||
                    block == BlockRegistry.ASH.get() ||
                    block == BlockRegistry.BRIGHT_SAND.get();
        });
    }
    ...
    private boolean place(IWorldGenerationReader generationReader, Random rand, BlockPos positionIn, Set<BlockPos> p_225557_4_, Set<BlockPos> p_225557_5_, MutableBoundingBox boundingBoxIn, BaseTreeFeatureConfig configIn) {
        ...
        if (blockpos.getY() >= 1 && blockpos.getY() + i + 1 <= 256) {
            if (!isValidToPlace(generationReader, blockpos.down())) {
                return false;
            } else {
    ...
    public final boolean func_230362_a_(ISeedReader p_230362_1_, StructureManager p_230362_2_, ChunkGenerator p_230362_3_, Random p_230362_4_, BlockPos p_230362_5_, BaseTreeFeatureConfig p_230362_6_) {
        Set<BlockPos> set = Sets.newHashSet();
        Set<BlockPos> set1 = Sets.newHashSet();
        Set<BlockPos> set2 = Sets.newHashSet();
        MutableBoundingBox mutableboundingbox = MutableBoundingBox.getNewBoundingBox();
        boolean flag = this.place(p_230362_1_, p_230362_4_, p_230362_5_, set, set1, mutableboundingbox, p_230362_6_);
        if (mutableboundingbox.minX <= mutableboundingbox.maxX && flag && !set.isEmpty()) {
            if (!p_230362_6_.decorators.isEmpty()) {
                ...
                p_230362_6_.decorators.forEach((p_236405_6_) -> {
                    p_236405_6_.func_225576_a_(p_230362_1_, p_230362_4_, list, list1, set2, mutableboundingbox);
                });
            }

            VoxelShapePart voxelshapepart = this.func_236403_a_(p_230362_1_, mutableboundingbox, set, set2);
            Template.func_222857_a(p_230362_1_, 3, voxelshapepart, mutableboundingbox.minX, mutableboundingbox.minY, mutableboundingbox.minZ);
            return true;
        } else {
            return false;
        }
