...
public class AlphaBiomeFeatures {
    ...
    // creating an empty tree config just so i can use the alter ground config
    public static final SphereReplaceConfig END_TERRAIN_CONFIG = (new SphereReplaceConfig(BlockRegistry.END_GRASS_BLOCK.get().getDefaultState(), 4, 1, Lists.newArrayList(Blocks.END_STONE.getDefaultState(), BlockRegistry.END_GRASS_BLOCK.get().getDefaultState())));
    ...
    public static void addMoltenTerrain(Biome biomeIn){biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.withConfiguration(new OreFeatureConfig(ASH, Blocks.MAGMA_BLOCK.getDefaultState(), 24)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(20, 8, 0, 256))));
        biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.withConfiguration(new OreFeatureConfig(ASH, BlockRegistry.DARK_DIRT_BLOCK.get().getDefaultState(), 32)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(20, 8, 0, 256))));
    }
