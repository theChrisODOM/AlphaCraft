...
public class AlphaBiomeFeatures {
    ...
    public static final HugeFungusConfig SMOOTH_OBSIDIAN_SPIKE = new HugeFungusConfig(BlockRegistry.BRIGHT_SAND.get().getDefaultState(), BlockRegistry.SMOOTH_OBSIDIAN.get().getDefaultState(), Blocks.AIR.getDefaultState(), Blocks.SHROOMLIGHT.getDefaultState(), false);
    ...
    public static void addSmoothObsidianSpikes(Biome biomeIn){
        biomeIn.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.field_236281_L_.withConfiguration(SMOOTH_OBSIDIAN_SPIKE).withPlacement(Placement.COUNT_HEIGHTMAP.configure(new FrequencyConfig(1))));
    }
    ...
    public static void addAlphaOres(Biome biomeIn) {
        biomeIn.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.withConfiguration(new OreFeatureConfig(DARK_STONE, BlockRegistry.PINK_DIAMOND_ORE.get().getDefaultState(), 8)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(5, 0, 0, 20))));
    }
    public static void addAlphaCarvers(Biome biomeIn) {
        biomeIn.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(AlphaCraftBiomesManager.ALPHA_CAVE.get(), new ProbabilityConfig(0.09F)));
    }
}